...
- [x] **1.7** Generate the Solidity verifier via `bb write_solidity_verifier`; commit `contracts/Verifier.sol` (generated, not hand-edited).
  _Commit: "Generate Solidity verifier contract"_
  _Generated with bb 5.1.0 at `-t evm` (keccak, ZK) to match proof.ts. Chain: circuit bytecode → `write_vk` → `write_solidity_verifier`; VK_HASH in the contract matches the committed `vk_hash`. Contract `HonkVerifier`, interface `verify(bytes, bytes32[]) view returns (bool)`, `NUMBER_OF_PUBLIC_INPUTS = 10` (circuit's 2 + UltraHonk-ZK extras — matters for 1.8/Phase 2 wiring), needs solc ≥ 0.8.27. Sanity-compiled with npx solc 0.8.28 + optimizer → clean (HonkVerifier bytecode emitted). Provenance + regen commands documented in `contracts/README.md`._
